...
function Page() {
  const t = useTranslations("likedPage");
    ...
    async (accID) => {
      ...
      try {
        const res = await axios.get(
          `${domain}/api/likes/${accID}?page=${page}`
        );
        if (res.status === 200) {
  ...
  const removeLiked = async (proid) => {
    if (!account) return;
    const isComfirm = confirm(
      "Are you sure that you want to remove like from this post ?"
    );
    if (!isComfirm) return;
    setIsPending(true);
    try {
      ...
      if (res.status === 200) {
        ...
        setTimeout(() => {
          alert("Removed like successfully!");
        }, 200);
      }
    } catch (error) {
      setIsPending(false);
      setTimeout(() => {
        alert("Failed to remove like, Try again later!");
      }, 100);
    }
  };
      ...
      <div className="max-w-[820px] mx-auto space-y-3.5 lg:pt-5 px-4 lg:px-0">
        ...
        {products.length === 0 && hasMore ? (
          ...
          <div className="lg:bg-white lg:pt-[16px] pb-[30px] px-2 lg:px-4 lg:border rounded w-full h-[350px] lg:h-[417.11px] flex flex-col font-sans">
            <div className=" font-semibold text-xl text-gray-800 hidden lg:block font-battambang">
              {t("title")}
            </div>
